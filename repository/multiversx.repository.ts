...
export class MultiversXRepository<T> implements IRepository<T> {
  ...
  apiEndpoint: string = process.env.NEXT_PUBLIC_MVX_ENDPOINT ?? 'https://api.multiversx.com'
  constructor() {
  ...
  async addWallet(connectedWallet: ConnectedWallet, wallet: Wallet): Promise<{ txn: string; persona: Persona }> {
    const txHash: Uint8Array = await this.personaSC.addWallet(new Address(connectedWallet.address), wallet)
  ...
  async removeWallet(connectedWallet: ConnectedWallet, wallet: Wallet): Promise<{ txn: string; persona: Persona }> {
    const txHash: Uint8Array = await this.personaSC.removeWallet(new Address(connectedWallet.address), wallet)
    const persona: Persona = await this.personaSC.getPersonaByWallet(connectedWallet.address)
