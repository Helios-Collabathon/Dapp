...
export default function PersonaPage() {
  ...
  const fetchINJBalance = async (prsn: Persona | undefined) => {
    if (!prsn) return;
    let _injBalance = 0;
    for (const wallet of prsn?.linked_wallets.filter(
      (wallet) => wallet.chain === Chain.Injective,
    )) {
      if (!wallet.verified) continue;
      const inj_toadd = await personaService.getBalance(
        wallet.address!,
        wallet.chain!,
      );
    ...
    if (connectedWallet.chain === Chain.Injective)
      _injBalance += await personaService.getBalance(
        connectedWallet.address,
        connectedWallet.chain,
      );
    return _injBalance;
  ...
  const fetchMVXBalance = async (prsn: Persona | undefined) => {
    ...
    let _mvxBalance = 0;
    for (const wallet of prsn?.linked_wallets.filter(
      (wallet) => wallet.chain === Chain.MultiversX,
    )) {
      if (!wallet.verified) continue;
      const mvx_toadd = await personaService.getBalance(
        wallet.address!,
        wallet.chain!,
      );
    ...
    if (connectedWallet.chain === Chain.MultiversX)
      _mvxBalance += await personaService.getBalance(
        connectedWallet.address,
        connectedWallet.chain,
      );
  ...
  return (
    <div className="relative flex w-full flex-col p-4 overflow-x-hidden">
      {loading && (
      ...
      <div
        className={
          loading
            ? "blur-lg filter transition duration-300 ease-in-out"
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4">
              <div className="mb-8 flex flex-col gap-2">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  />
                  <p className="font-mono text-xs w-full">
                    {isMobile
                      ? `${connectedWallet.address.slice(0, 5)}...${connectedWallet.address.slice(-5)}`
                      : connectedWallet.address}
                  </p>
                </div>
              </div>
              <div className="mb-8 flex flex-col gap-2 w-fit">
                ...
                <div className="flex gap-2 items-center">
                  <Image
                    ...
                    height={16}
                    alt="chain-sel-logo"
                    src={ChainUtils.getLogo(Chain.Injective)}
                  />
                  <div className="grid w-full grid-cols-2 place-content-center">
                    ...
                    <p className="text-xs font-light">
                      ≃ ${injBalance.usdt.toFixed(2)} $USDT
                    </p>
                  </div>
                </div>
                <div className="flex gap-2 items-center">
                  <Image
                    ...
                    height={16}
                    alt="chain-sel-logo"
                    src={ChainUtils.getLogo(Chain.MultiversX)}
                  />
                  <div className="grid w-full grid-cols-2 place-content-center">
                    <p className="w-full text-xs font-light">{`${mvxBalance.coin.toFixed(3)} $EGLD`}</p>
                    <p className="flex justify-end text-xs font-light">
                      ≃ ${mvxBalance.usdt.toFixed(2)} $USDT
                    </p>
                  </div>
                </div>
                <div>
                  <p className="w-full text-xs font-semibold">
                    Total: {(mvxBalance.usdt + injBalance.usdt).toFixed(2)}{" "}
                    $USDT
                  </p>
                </div>
              </div>
