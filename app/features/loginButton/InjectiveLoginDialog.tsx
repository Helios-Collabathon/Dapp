...
export const InjectiveLoginDialog = ({ isOpen, onClose }: InjectiveLoginDialogProps) => {
  ...
  const handleWalletClick = async (walletValue: Wallet) => {
    try {
      ...
      await wallet.connectWallet(address, Chain.Injective, walletValue.toString())
      if (!window.location.href.includes('persona')) window.location.href = `${window.location.origin}/persona`
    ...
    <>
      {isOpen && (
        <Dialog open={isOpen} onClose={onClose}>
          <div className="p-2">
            {/* Title Section */}
            <h1 className="mb-4 text-center text-2xl font-bold text-black dark:text-white">Injective Wallet Connection</h1>

            {/* Button Section */}
            <div className="grid grid-cols-2 gap-2">
              {wallets.map((wlt) => (
                <Button outline key={wlt.name} onClick={() => handleWalletClick(wlt.value)}>
